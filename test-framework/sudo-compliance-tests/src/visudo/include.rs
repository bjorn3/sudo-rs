...
fn prompt() -> Result<()> {
    let env = Env("@include sudoers2")
        .file("/usr/local/etc/sudoers2", "")
        .file(DEFAULT_EDITOR, TextFile(EDITOR_TRUE).chmod(CHMOD_EXEC))
        .build()?;

    let output = Command::new("visudo").output(&env)?;

    assert_contains!(output.stdout()?, "press return to edit /usr/local/etc/sudoers2:");
...
@include sudoers3",
    ...
    .file("/usr/local/etc/sudoers3", "# 3")
    .file("/usr/local/etc/sudoers4", "# 4")
    .file(
        DEFAULT_EDITOR,
        TextFile(format!(
            "#!/bin/sh
...
cat $2 >> {LOGS_PATH}"
    ...
    assert_contains!(
        output.stderr(),
        "/usr/local/etc/sudoers2: too many levels of includes"
    );
...
@includedir /usr/local/etc/sudoers.d")
    .file("/usr/local/etc/sudoers.d/a", "# 2")
    .file(
        DEFAULT_EDITOR,
        TextFile(format!(
            "#!/bin/sh
