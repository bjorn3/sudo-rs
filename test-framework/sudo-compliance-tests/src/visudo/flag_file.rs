...
use crate::{
    visudo::{CHMOD_EXEC, EDITOR_TRUE, ETC_SUDOERS, LOGS_PATH, TMP_SUDOERS},
...
fn creates_sudoers_file_with_default_ownership_and_perms_if_it_doesnt_exist() -> Result<()> {
    let env = visudo_env("", TextFile(EDITOR_TRUE).chmod(CHMOD_EXEC)).build()?;
...
fn regular_user_can_create_file() -> Result<()> {
    let env = visudo_env("", TextFile(EDITOR_TRUE).chmod("111"))
        .user(USERNAME)
        .build()?;
