...
fn relative_path() -> Result<()> {
    let env = Env("@include sudoers2")
        .file("/usr/local/etc/sudoers2", SUDOERS_ALL_ALL_NOPASSWD)
        .build()?;
...
fn include_loop_error_messages() -> Result<()> {
    let env = Env("@include /usr/local/etc/sudoers2")
        .file(r#"/usr/local/etc/sudoers2"#, "@include /usr/local/etc/sudoers")
        .build()?;
...
fn permissions_check_not_fatal() -> Result<()> {
    let env = Env([SUDOERS_ALL_ALL_NOPASSWD, "@include sudoers2"])
        .file(r#"/usr/local/etc/sudoers2"#, TextFile("").chmod("777"))
        .build()?;
...
fn relative_path_grandparent_directory() -> Result<()> {
    // base path is `/usr/local/etc/sudoers` so grandparent does not exist
