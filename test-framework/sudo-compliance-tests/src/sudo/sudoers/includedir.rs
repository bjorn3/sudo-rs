...
fn absolute_path() -> Result<()> {
    let env = Env("@includedir /usr/local/etc/sudoers.d")
        .file("/usr/local/etc/sudoers.d/a", SUDOERS_ALL_ALL_NOPASSWD)
        .build()?;
...
fn ignores_files_with_names_that_contain_a_dot() -> Result<()> {
    let env = Env("@includedir /usr/local/etc/sudoers.d")
        .file("/usr/local/etc/sudoers.d/a.", SUDOERS_ALL_ALL_NOPASSWD)
        .file("/usr/local/etc/sudoers.d/.b", SUDOERS_ALL_ALL_NOPASSWD)
        .file("/usr/local/etc/sudoers.d/c.d", SUDOERS_ALL_ALL_NOPASSWD)
        .build()?;
...
fn loads_files_in_lexical_order() -> Result<()> {
    let env = Env("@includedir /usr/local/etc/sudoers.d")
        .file("/usr/local/etc/sudoers.d/a", "ALL ALL=(ALL:ALL) ALL")
        .file("/usr/local/etc/sudoers.d/b", "ALL ALL=(ALL:ALL) NOPASSWD: ALL")
        .user(USERNAME)
        .build()?;
...
fn ignores_and_warns_about_file_with_bad_perms() -> Result<()> {
    let env = Env([SUDOERS_USER_ALL_NOPASSWD, "@includedir /usr/local/etc/sudoers.d"])
        .file(
            "/usr/local/etc/sudoers.d/a",
    ...
    assert!(output.status().success());
    let diagnostic = if sudo_test::is_original_sudo() {
        "/usr/local/etc/sudoers.d/a is world writable"
    } else {
        "/usr/local/etc/sudoers.d/a cannot be world-writable"
    };
    assert_contains!(output.stderr(), diagnostic);
...
fn ignores_and_warns_about_file_with_bad_ownership() -> Result<()> {
    ...
    assert!(output.status().success());
    let diagnostic = if sudo_test::is_original_sudo() {
        "/usr/local/etc/sudoers.d/a is owned by uid 1000, should be 0"
    } else {
        "/usr/local/etc/sudoers.d/a must be owned by root"
    };
    assert_contains!(output.stderr(), diagnostic);
...
fn include_loop() -> Result<()> {
    let env = Env([SUDOERS_USER_ALL_NOPASSWD, "@includedir /usr/local/etc/sudoers.d"])
        .file("/usr/local/etc/sudoers.d/a", TextFile("@include /usr/local/etc/sudoers.d/a"))
        .user(USERNAME)
        .build()?;
...
fn statements_prior_to_include_loop_are_evaluated() -> Result<()> {
    let env = Env([SUDOERS_USER_ALL_ALL, "@includedir /usr/local/etc/sudoers.d"])
        .file(
            "/usr/local/etc/sudoers.d/a",
            TextFile(format!(
...
@include /usr/local/etc/sudoers.d/a"
            )),
        )
        .user(USERNAME)
        .build()?;
...
fn no_hostname_expansion() -> Result<()> {
    let hostname = "ship";
    let env = Env("@includedir /usr/local/etc/sudoers.%h")
        .directory(format!("/usr/local/etc/sudoers.{hostname}"))
        .file(
            format!("/usr/local/etc/sudoers.{hostname}/a"),
...
fn ignores_directory_with_bad_perms() -> Result<()> {
    let env = Env("@includedir /usr/local/etc/sudoers2.d")
        .directory(Directory("/usr/local/etc/sudoers2.d").chmod("777"))
        .file("/usr/local/etc/sudoers2.d/a", SUDOERS_ALL_ALL_NOPASSWD)
        .build()?;
    ...
    let diagnostics = if sudo_test::is_original_sudo() {
        [
            "sudo: /usr/local/etc/sudoers2.d is world writable",
            "root is not in the sudoers file",
        ]
    } else {
        [
            "sudo-rs: /usr/local/etc/sudoers2.d cannot be world-writable",
            "authentication failed",
        ]
    };
...
fn ignores_directory_with_bad_ownership() -> Result<()> {
    let env = Env("@includedir /usr/local/etc/sudoers2.d")
        .directory(Directory("/usr/local/etc/sudoers2.d").chown(USERNAME))
        .file("/usr/local/etc/sudoers2.d/a", SUDOERS_ALL_ALL_NOPASSWD)
        .user(USERNAME)
        .build()?;
    ...
    let diagnostics = if sudo_test::is_original_sudo() {
        [
            "sudo: /usr/local/etc/sudoers2.d is owned by uid 1000, should be 0",
            "root is not in the sudoers file",
        ]
    } else {
        [
            "sudo-rs: /usr/local/etc/sudoers2.d must be owned by root",
            "authentication failed",
        ]
    };
