FROM dougrabson/freebsd14-small:latest
RUN IGNORE_OSVERSION=yes pkg install -y sshpass rsyslog
WORKDIR /usr/src/sudo
COPY target/build build
# set setuid on install
RUN install -m 4755 build/sudo /usr/bin/sudo && ls -l /usr/bin/sudo
RUN install -m 4755 build/su /usr/bin/su && ls -l /usr/bin/sudo
RUN install -m 755 build/visudo /usr/sbin/visudo
